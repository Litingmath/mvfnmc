...
class MvFNMC1:
    ...
    def __init__(self, lambda_d=2.5,  alpha=1, gamma=1.1, max_iter=10, k_p=3, K=5, eta=0.7, pre=1):
    ...
    def PLiBCD(self,test_idx,W1,W2,L1,L2,y):
        ...
        if self.max_iter==0:
            ...
            for i in range(self.max_iter):
                ...
                # Ds4=self.getGipKernel(1)
                # Ts4=self.getGipKernel(2)
                # W1[3]=self.preprocess_PNN(Ds4)
                # W2[3]=self.preprocess_PNN(Ts4)
    ...
    def __str__(self):
        return "Model: MvFNMC1, lambda_d:%s,  gamma:%s, alpha:%s, max_iter:%s, k_p:%s, K:%s, eta:%s, pre:%s" % (self.lambda_d,  self.gamma, self.alpha, self.max_iter, self.k_p, self.K, self.eta, self.pre)
