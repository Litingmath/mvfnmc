...
class MvFNMC1new1:
    ...
    def calculate_weights(self, F, Ld, Lt):
        ...
        for i in range(self.nd):
            ...
            self.wei_tar[i]=(1 / self.nd) + (sum_rvt - self.nd * rvt[i]) / (self.nd * self.gamma_t * (self.nd* max_rvt - sum_rvt))
    ...
    def PLiBCD(self,test_idx,W1,W2,LD,LT,y,sdf, stf):
        ...
        # self.wei_drug=[0,0,0,1]
        # self.wei_tar=[0,0,0,1]
        Ld=LD.copy()
        Lt=LT.copy()
        WD=W1.copy()
        WT=W2.copy()
        ...
        if self.max_iter==0:
            ...
            for i in range(self.max_iter):
                ...
                # mvfnmc
                if num_1 <= num_2:
                    AA = lambda1 * L1 + (1 + ala) * np.eye(num_1)
                    BB = lambda2 * L2
                else:
                    AA = lambda1 * L1
                    BB = lambda2 * L2 + (1 + ala) * np.eye(num_2)
