...
class MvFNMC2old:
    ...
    def __str__(self):
        return "Model: MvFNMC2old, lam:%s, lambda_d:%s,  gamma_d:%s, gamma_t:%s, alpha:%s, max_iter:%s, k_p:%s, K:%s, eta:%s, pre:%s, delay:%s" % (self.lam, self.lambda_d,  self.gamma_d, self.gamma_t, self.alpha, self.max_iter, self.k_p, self.K, self.eta, self.pre, self.delay)
