...
class NMC2:
    ...
    def PLiBCD(self,test_idx,W1,W2,LD,LT,y,sdf, stf):
        ...
        '''
        self.wei_drug=[0,1,0,0]
        self.wei_tar=[0,1,0,0]
        Ld=LD.copy()
        Lt=LT.copy()
        WD=W1.copy()
        WT=W2.copy()
    ...
    def __str__(self):
        return "Model: NMC2, lam:%s, lambda_d:%s,  gamma_d:%s, gamma_t:%s, alpha:%s, max_iter:%s, k_p:%s, K:%s, eta:%s, pre:%s, delay:%s" % (self.lam, self.lambda_d,  self.gamma_d, self.gamma_t, self.alpha, self.max_iter, self.k_p, self.K, self.eta, self.pre, self.delay)
