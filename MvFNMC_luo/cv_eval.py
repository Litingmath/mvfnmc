...
def cv_eval_iter(output_dir,method, dataset, cv_data, cvs, args,Dm3,Tm3):
    max_aupr, aupr_opt, results_list = 0, [], []
    for iter in np.arange(1,15):
        tic = time.clock()
        if method=='svnmc':
            model = SvNMC(lambda_d=args['lambda_d'],  alpha=args['alpha'], max_iter=iter, K=args['K'], eta=args['eta'], pre=args['pre'])
        if method== 'mvfnmc1new1':
            model = MvFNMC1new1(lambda_d=args['lambda_d'],  alpha=args['alpha'], gamma_d=args['gamma_d'], gamma_t=args['gamma_t'], max_iter=iter, k_p=args['k_p'],  K=args['K'], eta=args['eta'], pre=args['pre'], delay=args['delay'])
        if method== 'mvfnmc2':
            model = MvFNMC2(lam=args['lam'], lambda_d=args['lambda_d'],  alpha=args['alpha'], gamma_d=args['gamma_d'], gamma_t=args['gamma_t'], max_iter=iter, k_p=args['k_p'],  K=args['K'], eta=args['eta'], pre=args['pre'], delay=args['delay'])
