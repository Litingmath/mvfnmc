...
class MvFNMC1new:
    ...
    def calculate_weights(self, F, Ld, Lt):
        ...
        max_rvt=np.max(rvt)
        for i in range(self.nd):
            self.wei_drug[i]=(1 / self.nd) + (sum_rud - self.nd * rud[i]) / (self.nd * self.gamma * (self.nd* max_rud - sum_rud))
    
            self.wei_tar[i]=(1 / self.nd) + (sum_rvt - self.nd * rvt[i]) / (self.nd * self.gamma * (self.nd* max_rvt - sum_rvt))
    ...
    def PLiBCD_L2(self,test_idx,W1,W2,y,InitZ):
    ...
    def PLiBCD(self,test_idx,Dm3,Tm3,y):
        ...
        self.T3=Tm3
        # ytrain
        sd4=self.GIP_kernel(y) 
        st4=self.GIP_kernel(y.T) 
        
        W1=np.concatenate((self.D3, sd4[np.newaxis, :, :]), axis=0)
        W2=np.concatenate((self.T3, st4[np.newaxis, :, :]), axis=0)
        ...
        # new sim_matrix
        # sd4=self.GIP_kernel(self.Z) 
        # st4=self.GIP_kernel(self.Z.T) 
        # W1=np.concatenate((self.D3, sd4[np.newaxis, :, :]), axis=0)
        # W2=np.concatenate((self.T3, st4[np.newaxis, :, :]), axis=0)

        Ld[0]= self.compute_normalized_laplacian(self.D3[0])
        Ld[1]= self.compute_normalized_laplacian(self.D3[1])
        Ld[2]= self.compute_normalized_laplacian(self.D3[2])
        Ld[3]= self.compute_normalized_laplacian(W1[3])
        
        Lt[0]= self.compute_normalized_laplacian(self.T3[0])
        Lt[1]= self.compute_normalized_laplacian(self.T3[1])
        Lt[2]= self.compute_normalized_laplacian(self.T3[2])
        Lt[3]= self.compute_normalized_laplacian(W2[3])
    ...
    def fix_model(self, test_idx,Dm3,Tm3,y):
     
        self.PLiBCD(test_idx,Dm3,Tm3,y)
    ...
    def __str__(self):
        return "Model: MvFNMC1new, lambda_d:%s,  gamma:%s, alpha:%s, max_iter:%s, K:%s, eta:%s, pre:%s" % (self.lambda_d,  self.gamma, self.alpha, self.max_iter, self.K, self.eta, self.pre)
