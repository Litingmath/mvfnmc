...
class MvFNMC1new1:
    ...
    def __init__(self, lambda_d=2.5,  alpha=1, gamma_d=1.1, gamma_t=1.1,max_iter=10, k_p=3, K=5, eta=0.7, pre=1, delay=10, cvs=1):
    ...
    def PLiBCD(self,test_idx,W1,W2,LD,LT,y):
        ...
        if self.max_iter==0:
            ...
            for i in range(self.max_iter):
                ...
                # print(L1.shape)
                self.calculate_weights(LapA, Ld, Lt) # 更新权重
    ...
    def fix_model(self, test_idx,WD,WT,LD,LT,y):
        ...
        self.PLiBCD(test_idx,WD,WT,LD,LT,y) # WD, WT 不保留由DTI邻接矩阵产生的相似性
    ...
    def __str__(self):
        return "Model: MvFNMC1new1, lambda_d:%s,  gamma_d:%s, gamma_t:%s,alpha:%s, max_iter:%s, k_p:%s, K:%s, eta:%s, pre:%s, delay:%s" % (self.lambda_d,  self.gamma_d, self.gamma_t, self.alpha, self.max_iter, self.k_p, self.K, self.eta, self.pre, self.delay)
